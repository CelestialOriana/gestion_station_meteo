...
<head>
    ...
    // Affichage pour débogage, peut être retiré une fois testé
    echo "Id station: " . htmlspecialchars($ValId_station) . "<br>";
    echo "Jour: " . htmlspecialchars($ValJour) . "<br>";
    echo "Température: " . htmlspecialchars($ValTemp) . "<br>";
    echo "Ensoleillement: " . htmlspecialchars($ValEns) . "<br>";

    // Vérifier que la date (jour) est supérieure ou égale à 24
    if ($ValJour < 24) {
        echo "Erreur : La date doit être supérieure ou égale à 24.<br>";
    } else {
        // Préparer la requête INSERT INTO
        $requete = $Bdd->prepare('INSERT INTO mesures (temperature, ensoleillement, date, id_station) 
                                 VALUES (:f1, :f2, :f3, :f4)');
